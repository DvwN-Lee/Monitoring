...
## AI Agent 확장 설계 (향후 구현 예정)

> **이 섹션은 향후 구현 예정인 설계 문서입니다.** 현재 코드베이스에는 포함되어 있지 않으며, 기존 모니터링 인프라를 AI Agent의 Tool로 활용하기 위한 아키텍처 설계를 정리한 것입니다.

### 설계 배경

이 프로젝트의 `/stats` 집계, 서비스 상태 감지, WebSocket 하트비트 등 **이미 구축된 관측 인프라**는 AI Agent가 Tool로 활용하기에 적합한 구조입니다. 이 데이터 소스들을 MCP(Model Context Protocol) Server로 래핑하고 LangGraph 기반 Agent가 호출하면, 장애 감지 → 원인 분석 → 조치까지의 운영 워크플로우를 자동화할 수 있습니다.

핵심 아이디어는 **기존 마이크로서비스 인프라 자체가 AI Agent의 Tool이자 Data Source가 되는 것**입니다.

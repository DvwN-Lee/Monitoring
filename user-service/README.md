...
## 2. 핵심 기능 및 책임
- **사용자 계정 관리**: 사용자 생성(회원가입) 및 사용자 이름 기반의 사용자 정보 조회 기능을 제공
...
- 이 서비스의 `/stats` 엔드포인트는 단순한 상태 정보(`online`)만 반환하는 다른 서비스와 달리, 자신이 의존하는 **데이터 저장소의 상태를 직접 점검**하여 반환
    ...
    - DB 또는 캐시 중 하나라도 비정상이면 전체 `service_status`를 `degraded`로, 둘 다 정상이면 `online`으로 반환하며, 캐시 응답에는 `hit_ratio` 필드도 포함된다

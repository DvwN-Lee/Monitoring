...
2.  **하위 서비스 상태 병렬 수집**:
    - `api-gateway`, `user-service`, `auth-service`, `blog-service` 4개 하위 서비스의 `/stats` URL 목록을 정의
...
- **`/api/ws-heartbeat` 핸들러**:
    ...
    - 다음 세 가지 시점에 현재 시간을 `wsActivities` 슬라이스에 기록:
        1. WebSocket 연결이 최초 수립될 때
        2. 서버가 5초 간격으로 전송하는 Ping에 대한 Pong 응답을 수신할 때 (`PongHandler`)
        3. 클라이언트(`dashboard-ui`)로부터 메시지를 수신할 때
